...
def main(gmp: Gmp, args: Namespace) -> None:
    ...
    # Exibe no terminal com pretty_print (continua igual)
    #pretty_print(report_element)

    # Novo: salva o conteúdo do pretty_print em arquivo separado
    rough_string = ET.tostring(report_element, 'utf-8')
    reparsed = minidom.parseString(rough_string)
    pretty_xml = reparsed.toprettyxml(indent="  ")

    # Caminho do arquivo para o pretty XML
    pretty_output_path = Path("pretty_" + xml_filename).expanduser()
    pretty_output_path.write_text(pretty_xml, encoding="utf-8")
    print("Pretty-printed XML salvo em:", pretty_output_path)
